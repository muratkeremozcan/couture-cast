...
describe('5.4 palette advisor locale parity (web)', () => {
  it('5.4-I18N-WEB-01 ships a palette tree for every supported locale', () => {
    for (const locale of SUPPORTED_LOCALES) {
      expect(Object.keys(paletteTree(CATALOGS[locale])).length, locale).toBeGreaterThan(0)
    }
  })

  it('5.4-I18N-WEB-02 uses an identical key tree in every locale', () => {
   ...
   */
  it('5.4-I18N-WEB-03 carries exactly the keys the contract enumerates', () => {
    expect([...reference.keys()].sort()).toEqual(CONTRACT_KEYS)
  })

  it('5.4-I18N-WEB-04 keeps interpolation placeholders identical across locales', () => {
    for (const locale of SUPPORTED_LOCALES) {
  ...
  it('5.4-I18N-WEB-05 does not leave English values in non-English catalogs', () => {
  ...
  it('5.4-I18N-WEB-06 never ships an empty string', () => {
    for (const locale of SUPPORTED_LOCALES) {
   ...
   * actually uses.
   */
  it('5.4-I18N-WEB-07 keeps the sponsored disclosure substantive in every locale', () => {
   ...
   */
  it('5.4-I18N-WEB-08 keeps the consent copy substantive in every locale', () => {
    for (const locale of SUPPORTED_LOCALES) {
      const catalog = flatten(paletteTree(CATALOGS[locale]))
   ...
   */
  it('5.4-I18N-WEB-09 uses Canadian spellings in en-CA and American ones in en-US', () => {
