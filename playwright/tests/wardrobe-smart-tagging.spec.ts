...
test.describe('Wardrobe Smart Tagging Flow', () => {
  ...
  test('[P0] [4.2-E2E-001] uploads, resumes, confirms, reloads, and uses the garment in a Ritual', async ({
    apiRequest,
    page,
    recurse,
  }, testInfo) => {
    const accessToken = await test.step('Create an isolated wardrobe user', async () => {
      ...
      cleanupUserId = signupBody.userId
      const token = `test-token:guardian:${signupBody.userId}`
      cleanupAccessToken = token
    ...
    await test.step('Override the suggestion with the keyboard and save', async () => {
      const topChip = page.getByRole('radio', { name: /Top/ })
      await topChip.focus()
      await expect(topChip).toBeFocused()
      await topChip.press('ArrowRight')
      const bottomChip = page.getByRole('radio', { name: /Bottom/ })
