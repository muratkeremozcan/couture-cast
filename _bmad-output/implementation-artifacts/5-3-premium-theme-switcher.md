...
- [ ] **Task 6 — Mobile (AC 1, 2, 4, 5, 6, 7)** — **CANCELLED for this pass.** The mobile surface was cut so the higher test tiers could be authored separately; nothing under `apps/mobile` is touched. Recorded in `deferred-work.md` under "Deferred from: story 5.3 premium theme switcher", which also carries the cancelled mobile halves of AC 4, AC 6 and AC 7.
  - [ ] `theme-palettes.ts`, `theme-context.tsx` (`AppThemeProvider`/`useAppTheme`, name-collision-safe per Decision 12), mounted in `_layout.tsx`
  - [ ] `premium-theme.ts` lib, `PremiumThemeSection` inline in `settings.tsx` per Decision 12
  - [ ] Locale keys (Decision 13) + `premium-theme-locales.spec.ts`; screen tests with MSW handlers
- [ ] **Task 7 — Pact + Playwright (AC 1-6)** — **CANCELLED for this pass**, along with Maestro and the Decision 3 adapter rewrite in `playwright/support/helpers/accessibility.ts`. Same reason and same ledger entries as Task 6. The k6 line below stands as written: it was never in scope.
  - [ ] Pact interactions in `api-contract-interactions.ts`: mobile + web both call GET and PUT (unlike 5.2's asymmetric split, both consumers use both operations here) — `200`/`403`/`503` states; provider states `'The user has premium theme access'`, `'The user does not have premium theme access'`, `'Premium themes are disabled'`
  - [ ] Playwright `premium-theme-switcher.spec.ts`: locked state signed-out (+ axe), gallery + select + persist-on-reload for an entitled seeded user, Default fallback for a stale/unknown stored value (seed directly, don't rely on UI to produce that state)
  - [ ] **No k6 addition** — this is a low-QPS settings write, not a per-request-hot path like `subscriptionStatus`; do not reflexively copy 5.2's k6 task
