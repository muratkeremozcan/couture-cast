...
- [x] Task 11: Document CI/CD architecture
  - [x] Create `_bmad-output/test-artifacts/ci-cd-pipeline.md` with pipeline diagram
...
- [x] Task 12: Run Playwright smoke against Vercel Preview deployments (PRs) (AC: #3)
  ...
  - [x] Verify on a PR: Vercel Preview deploy completes → smoke workflow runs → `web-health-sha.spec.ts` passes

- [x] Task 13: Prepare API deployment on Vercel (serverless Nest adapter)
...
- [Architecture: ADR-010 Testing & CI](../planning-artifacts/architecture.md#architecture-decision-records-adrs)
...
- [Epics: Epic 0 Story CC-0.6](../planning-artifacts/epics.md#epic-0--platform-foundation--infrastructure-sprint-0)
- Baseline Reference: playwright-utils e2e-test.yml (local reference used during implementation; not committed as a portable repo link)
- Baseline Reference: playwright-utils burn-in workflow (local reference used during implementation; not committed as a portable repo link)
...
- `npm run typecheck` (workspace + Playwright TS)
