...
**Date:** 2025-11-13  
**Updated:** 2026-03-03 - Retained as active legacy baseline during BMAD v6 migration.  
**Author:** BMad-user  
**Status:** Active baseline (legacy format; pending split into v6 TEA outputs)
...
- Document any missing ADRs in `_bmad-output/project-knowledge/architecture/decisions/` before gate review
...
- **Maestro (mobile sanity + analytics advisory)**: minimal launch check via Expo Go (dev URL load + screenshot), plus diagnostics-mode analytics validation for `ritual_created`, `wardrobe_upload_started`, and `alert_received`. Env: `MOBILE_E2E_APP_URL` (defaults to `exp://10.0.2.2:8081/--/` on Android emulator and `exp://127.0.0.1:8081/--/` on iOS simulator), `MOBILE_E2E_HEALTH_URL` if overriding, and `MOBILE_ANALYTICS_DIAGNOSTICS=1` only for `maestro/analytics.yaml`. Treat Maestro as advisory until `_bmad-output/test-artifacts/maestro-analytics-promotion.md` thresholds are met.
  ...
  forbid: [
    {
      ...
      message: 'Hard waits forbidden - use waitForSelector or waitForResponse instead',
    },
  ],
})
```
