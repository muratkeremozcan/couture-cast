...
- **Color analysis pipeline:** ~~Committed to an in-house selfie-based palette detection service; confirm on-device processing constraints, privacy posture, and performance budgets before CC-5.4 to keep data inside CoutureCast.~~ **Resolved 2026-08-26 by ADR-014 and story 5.4.** It is an in-house service, and it is SERVER-SIDE rather than on-device: the question this line asked was answered by ADR-014, which rejected on-device on model size, inconsistent device performance and the inability to update classification without an app release, and story 5.4 implemented that. Privacy posture: the selfie is uploaded under a user-scoped path, decoded once, and PURGED on every terminal branch of the analysis (Decision 8), so only derived metadata — undertone, depth, confidence — is ever at rest; consent is explicit, separately granted, and revoking it erases the derived palette. Performance budget: the analysis runs on the BullMQ worker rather than in the request, so the client sees `processing` and polls, and no user-facing request is held open for it. Leaving this line asking a question that shipped code has answered would make a reader wrong about where face images are processed.
