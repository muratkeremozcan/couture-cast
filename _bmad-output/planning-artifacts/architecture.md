...
| Background jobs           | BullMQ workers + BullMQ 5 Job Schedulers                  | BullMQ 5.65 / standalone worker runtime          | 1‑6           | Unified queueing for weather, alerts, moderation                                    |
...
├─ _bmad-output/            # Project knowledge plus planning, implementation, and test artifacts
...
| ---------------------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 1 – Weather Intelligence & Platform Foundation | Weather ingestion, location prefs, alerts, telemetry    | `apps/api` weather module, BullMQ scheduler/jobs, Prisma weather tables, OpenTelemetry/Grafana |
...
- **Backend:** NestJS 11, Prisma 6, Node.js 24+, BullMQ 5, Socket.io 4, Supabase Postgres 15, Supabase Storage, Upstash Redis.
- **Tooling:** Turborepo + npm workspaces, Vitest, Playwright, Maestro, PostHog, OpenTelemetry/Grafana Cloud, Vercel Edge Config.

### Integration Points

- **Weather provider** (OpenWeather primary, WeatherAPI secondary) ingested at a configurable cadence no greater than 30 minutes via BullMQ Job Schedulers and the standalone worker runtime.
...
12. **ADR-012 Weather Provider Selection & Failover Strategy:** Primary provider OpenWeather One Call 4.0, with One Call 3.0 compatibility, returns current conditions and hourly forecast data in one provider call. WeatherAPI.com is the bounded secondary provider. The ingestion orchestrator owns retry and failover: primary `429` fails over immediately, retryable primary timeout/`5xx` failures use at most three primary attempts total (immediate, +5s, +15s), then one secondary attempt. BullMQ per-location jobs use `attempts: 1` so queue retries do not multiply provider calls. If both providers fail, latest-weather reads return the newest persisted snapshot using the freshness union: `fresh`, `cached`, `stale`, or `unavailable`, with `age > 60 minutes` classified as stale. BullMQ 5 Job Schedulers own the durable `weather-refresh-sweep` cadence in the standalone worker runtime; Vercel HTTP instances do not run weather cron. Provider adapters implement `IWeatherProvider.fetchForecast(target, signal)` and normalize metric, UTC, 48-hour payloads before transactional persistence. Operational telemetry is OpenTelemetry/Grafana only, with bounded provider/outcome/status attributes and a 2% five-minute provider error-rate alert.
...
14. **ADR-014 Color Analysis Architecture:** Server-side processing (not on-device) for wardrobe color extraction and palette analysis. NestJS worker receives upload webhook from Supabase Storage, processes via Sharp (image resize) → ONNX Runtime (color inference using pre-trained model), then caches results in `wardrobe_items.color_palette` JSONB column. Privacy: images stay in Supabase Storage with user-scoped access; only derived metadata (palette hex codes, undertone classification) surfaces to clients. Performance: ~3-5s per image; GPU fallback to CPU in CI environments. On-device processing rejected due to: (1) model size (50+ MB) would bloat mobile bundle, (2) inconsistent device performance (older Android devices timeout), (3) centralized processing enables palette quality monitoring and ML model updates without app releases. **Amended 2026-08-26 by story 5.4 (colour palette & beauty/accessory advisor), which is the first implementation of this ADR and diverges from it in one named step.** The location decision and the privacy posture are implemented exactly as written above — processing is server-side in the NestJS worker, images stay in Supabase Storage under user-scoped access, only derived metadata reaches a client — and the `refs/implementation-readiness-report-2025-11-13.md:242-245` blocker ("Architecture defines NestJS worker (server-side), but PRD implies potential on-device option") is discharged by that. **The divergence is the ONNX Runtime step, which story 5.4 declines.** The output the advisor needs is one four-way undertone classification and one five-way depth classification, which closed-form CIELAB colour science settles exactly and deterministically; a learned model would add a probabilistic answer to a question that has an arithmetic one. The cost is concrete rather than theoretical: this repository's only existing ONNX consumer (garment tagging) carries a 50 MB model directory, a `GARMENT_TAGGING_MODEL_DIR` cache and a `verify:tagging-model` prestart gate, all of which are worth paying for a genuinely learned task and none of which are worth paying for a hue-angle threshold. Sharp is used as specified, for decode and resize. **Two further clarifications this ADR did not anticipate.** First, the palette advisor stores its derived result on `PaletteProfile` rather than in `wardrobe_items.color_palette`: that JSONB column holds a garment's colours, and a person's undertone is a different subject with a different retention rule. Second, and the sharper one, the selfie source has a retention posture this ADR's "images stay in Supabase Storage" does not describe — story 5.4's Decision 8 PURGES the selfie object on every terminal branch of the analysis, success or failure, so a face image is never at rest after the analysis ends. That is deliberately stricter than this ADR, and it is what makes "there is no re-analysis without re-upload" true. A future story that wants to reintroduce the ONNX step, or to relax the purge, is reopening this ADR rather than merely implementing it.
15. **ADR-015 Billing & Entitlement Architecture (story 5.2):** RevenueCat is the single entitlement ledger (`react-native-purchases` v10.x on mobile: StoreKit 2 + Play Billing 8; entitlement id `premium`; `app_user_id` = our `User.id`), with our own Stripe account for web checkout (`stripe-node` v22.x, Checkout `mode: subscription`, Customer Portal for cancel/plan-switch) forwarded into RevenueCat via its Stripe integration, so entitlement truth has exactly ONE writer: the RevenueCat webhook (plus the refresh/reconciliation ledger pulls that share the same ordering guard). Our database mirrors, never originates, entitlement state. The vendor question the architecture reserved ("Payments/commerce reserved for future integration (Stripe)", honored for the web rail) was weighed on both principles: the free-tier-friendly launch posture favors RevenueCat (free to $2,500 MTR — noting forwarded Stripe revenue counts toward MTR), and the vendor-minimalist philosophy counts against a second vendor — accepted because RevenueCat replaces three verification stacks (Apple JWS, Google service-account JWT + a new Pub/Sub surface for RTDN, Stripe HMAC) and two webhook endpoints. RevenueCat Web Billing was rejected: it moves the checkout surface and billing relationship onto RevenueCat exactly where this design preserves reversibility. Reversal cost, honestly: the web half is bounded (subscriptions live in our Stripe account; reversal repoints entitlement writes at Stripe webhooks we already receive); the mobile half is a multi-release migration (the RC SDK in shipped binaries is the offerings source, purchase UI, and restore flow). The server contract (PremiumEntitlement mirror, status/refresh endpoints, PremiumEntitlementGuard, settings sections) is vendor-neutral by construction and survives either direction. Known degraded mode, named: "paid-but-locked" — an RC outage delays new activations while synced entitlements keep working from the mirror; the forward outbox holds the obligation durably, the 15-minute reconciliation sweep re-drives it (BullMQ Job Scheduler in the worker runtime per ADR-012's substrate — a serverless `@Cron` never fires in production), and the break-glass is an RC promotional grant. A Stripe-sourced provisional activation was rejected: dual-writer drift is worse than bounded delay. Accepted lag, surfaced in UX copy: Stripe-sourced cancellations can take ~2 hours to reflect. Privacy: `app_user_id` = raw `User.id` diverges from the PostHog HMAC-pseudonym discipline; weighed against a dedicated alias id and accepted because the webhook must map back to a user, RevenueCat is a processor under the settings disclosure (AC 7), and `PremiumEntitlementService.eraseProcessorData` deletes the RC subscriber and Stripe customer on account erasure. The controllable sync guarantee is "visible on demand + bounded client poll" (webhook write is synchronous; refresh pulls the ledger; clients poll 5s/2min) — provider webhook latency itself is monitored, not tested. First-party store APIs (App Store Server API + RTDN) remain the documented fallback path if this decision is reversed.
...
_Updated: 2026-08-12 — ADR-015 added for the story 5.2 billing and entitlement vendor decision_
_Updated: 2026-08-26 — ADR-014 amended by story 5.4: the ONNX Runtime step is declined in favour of closed-form CIELAB classification, the derived palette lands on `PaletteProfile` rather than `wardrobe_items.color_palette`, and the selfie source is purged on every terminal branch rather than retained_
